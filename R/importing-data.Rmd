...
dat <- read_csv(filename)
```
Note that we receive a message letting us know what data types were used for each column. Also note that dat is a tibble, not just a data frame. This
...
# 
 dlb<- list.files(path, pattern = ".csv")%>%
   # map_df(~read_csv(file.path(path,.)))
...
for (i in 1:length(temp)) assign(gsub(".csv","",temp[i]), read.csv(file.path(path,temp[i])))
...
```{r}
???
read_lines(file.path(path,"olive.csv"), n_max = 3)
 dlb<- list.files(path, pattern = ".csv")%>%
   # map_df(~read_csv(file.path(path,.)))
    map(~read_csv(file.path(path,.),name_repair="universal"))
