...
murders %>%
  mutate(rate=total/population_in_millions,
          rank=rank(-rate))
...
NHANES[AgeDecade==" 20-29",.(ref=meansd(BPSysAve))]
```

2\. Report the min and max values for the same group.

```{r}
NHANES[AgeDecade==" 20-29",.(min(BPSysAve,na.rm = T),max(BPSysAve,na.rm = T))]
```
...
```{r}
NHANES[,.(mean(BPSysAve)),by=AgeDecade]
```
